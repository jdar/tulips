...
echo 'alias tulips="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "git@github.com:jdar/tulips.git" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'git@github.com:jdar/tulips.git'"
...
rm -rf "$HOME/.config/@tulips"
rm -rf "$HOME/.config/@tulips"
rm -rf "$HOME/.config/tulips-core"
